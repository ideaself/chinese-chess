...
// ── 开局分类（实现在 openingClassify.ts，此处再导出保持既有引用） ──

import { classifyRecord, FAMILY_INFO } from './openingClassify'
import type { Classification, OpeningFamily, BlackDefense } from './openingClassify'

export type { OpeningFamily, BlackDefense, Classification } from './openingClassify'
export { FAMILY_INFO, DEFENSE_INFO, classifyRecord } from './openingClassify'

/** 已分类的棋谱库记录（分类结果随记录一起缓存） */
