 ...
 * 开局体系分类（纯函数，无数据加载）
 *
 * 从 masterLibrary.ts 抽出的分类逻辑：开局训练（openings.ts）与大师棋谱库
 * 都要用，放在独立模块可避免把棋谱库的加载代码打进主包。
 */
...
function classifyFamily(firstMove: string): OpeningFamily {
  ...
  // 飞相局：相三进五(c0e2) / 相七进五(g0e2)；c2e0 为旧数据遗留写法
  if (firstMove === 'c0e2' || firstMove === 'g0e2' || firstMove === 'c2e0') return 'feixiang'
